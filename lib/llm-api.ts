...
export const SCENE_PRESETS: Record<string, string> = {
  technical_params: `【场景：技术参数表】本场景翻译风格固定为严谨专业，以下约束优先级最高。所有数值、单位、符号原样保留，仅翻译说明文字。保持表格结构、条目顺序完全对应，不增删行项。术语极致严谨，不做任何意译与发挥。`,
  ecommerce: `【场景：电商详情页】卖点前置，短句为主，适合快速阅读。符合目标语言本土电商表达习惯，有感染力但不夸大。禁止直译中文网络热词、成语梗，做本土化意译。标题简洁有冲击力，详情段落逻辑清晰。`,
  packaging: `【场景：包装文案】本场景翻译风格固定为严谨专业，以下约束优先级最高。译文简洁，控制字符长度，适配印刷版面。合规信息准确，产地、质保、警示语符合当地法规。品牌、型号、规格位置固定，译文不遮挡核心标识。避免生僻词，确保普通消费者快速理解。`,
  ui: `【场景：软件UI】本场景翻译风格固定为严谨专业，以下约束优先级最高。按钮、菜单、弹窗控制在1~3个词，极致精简。统一使用指令式或名词式风格，前后一致。报错提示清晰易懂，不堆砌专业术语。预留文本膨胀空间，德语、荷兰语、波兰语等长词语言优先最短表达。`,
}

export const SCENE_PRESETS_EN: Record<string, string> = {
  technical_params: `[Scene: Technical Parameter Table] Style is locked to Professional. All values, units, and symbols must be preserved as-is. Translate only explanatory text. Maintain table structure and row order exactly — no adding or removing rows. Be rigorously precise with terminology — no paraphrasing or creative interpretation.`,
  ecommerce: `[Scene: E-commerce Product Page] Lead with key selling points. Use short sentences for quick scanning. Match the target market's native e-commerce expression style — compelling but not exaggerated. Never calque English marketing slang or idioms; adapt them natively. Titles should be punchy and concise; body copy should flow logically.`,
  packaging: `[Scene: Packaging Copy] Style is locked to Professional. Keep translations concise — control character length for print layout. Compliance info (origin, warranty, warnings) must be accurate per local regulations. Brand, model, and spec labels stay in fixed positions; translation must not obscure core identifiers. Avoid obscure vocabulary; ensure quick comprehension by general consumers.`,
  ui: `[Scene: Software UI] Style is locked to Professional. Buttons, menus, and toasts must be 1–3 words — ultra-compact. Use a consistent imperative or noun-based style throughout. Error messages should be clear and user-friendly — avoid piling up technical jargon. Reserve space for text expansion; for German, Dutch, Polish, and other long-word languages, prefer the shortest viable expression.`,
}
...
function getStylePrompt(config: LLMConfig, isEnSource?: boolean, scenePreset?: string): string {
  ...
  const isSceneForced = scenePreset === 'technical_params' || scenePreset === 'packaging' || scenePreset === 'ui'
