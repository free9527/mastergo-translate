...
const PRODUCT_LINE_STRATEGIES: Record<string, string> = {
  ...
  gaming_card: `【产品线：游戏存储卡】受众为Switch/Steam Deck/ROG Ally等掌机玩家。"高读写速度"→UHS-I/V30高读取速度确保游戏快速加载、关卡秒切、开放世界无缝读图，不因存储卡速度拖累游戏体验。"A2等级"→高随机读写IOPS提升掌机程序运行效率，游戏启动更快、运行更流畅、资源加载不卡顿。"大容量"→512GB/1TB海量空间，游戏库随身携带，告别反复删除重装的焦虑。文案简洁有活力，可用游戏圈表达（"加载碾压""随身游戏库"）。`,
...
const PRODUCT_LINE_STRATEGIES_EN: Record<string, string> = {
  ...
  gaming_card: `[Product Line: Gaming Card] Audience: Switch, Steam Deck, ROG Ally, and other handheld gamers. "High read/write speed" → UHS-I, V30 fast read speeds ensure quick game loading, instant level transitions, and seamless open-world streaming — never let the card bottleneck your gameplay. "A2 rating" → high random read/write IOPS boost app performance on handhelds: faster game launches, smoother in-game asset streaming, zero stutter. "Large capacity" → 512GB/1TB to carry your entire game library — no more deleting and reinstalling. Keep copy concise and energetic. May use gaming-native language ("blazing load times", "carry your game library").`,
