 ...
 */
export const PROHIBITED_WORDS_VERSION = 6  // v12.20: zh-TW「測試/最高」实机误报补漏（质量背书测试 + 规格测试名 + 最高理論頻寬）（词表维护纪律：任何增删 +1，否则旧译文缓存复活）
...
export const PROHIBITED_ZH_EXEMPTIONS: string[] = [
  ...
  '最大#',
  // ── v12.15：「最高」同型补漏（与「最大#」同纪律——数字锚定恒为规格上限）──
  // 实机实锤（2026-09-03 zh-TW 润色日志）：LLM 自然产出「最高寫入速度高達 1650MB/s」
  // 命中裸「最高」触发润色硬锁⑧层回退——但「最高+数字」是 up to 的规格上限表述，
  // 与已豁免的「最高可达」「最大#」同型。补数字锚定豁免 + 速度规格短语豁免：
  '最高達#',        // 最高達2TB/最高達12,000次（繁体——zh-TW 实机形态）
  '最高达#',        // 简体同型
  '最高讀取速度',   // 最高讀取速度1750MB/s（规格陈述）
  '最高寫入速度',   // 最高寫入速度1650MB/s
  '最高读取速度',
  '最高写入速度',
  // ── v12.20：zh-TW「測試/最高」实机误报补漏（三类锚定：加锚不删词，裸词红线仍命中）──
  // 背景（2026-09-14 实机实锤）：zh-TW 校对改写后仍命中「測試×4 / 最高×3 / 100%×1」，
  //   全是合规语境误报。根因：zh 豁免表对齐了 ja/en 的「テスト/test」质量背书与「最大#」
  //   数字锚，却漏了 zh-TW 词表独有的「測試」词（zh-CN 词表无「测试」，本组仅服务
  //   zh-TW 目标语检测）与「最高+带宽」规格形态。
  // ① 质量背书测试（锚定「嚴苛/大量/實驗室/內部」限定词——质量背书结论，非 test sample 声称）
  '嚴苛測試',       // 歷經嚴苛測試（质量背书）
  '大量測試',       // 經過大量測試（质量背书）
  '實驗室測試',     // 實驗室測試（质量背书）
  '內部測試',       // 內部測試（质量背书）
  // ② 规格测试名（锚定测试类型名——对齐 en SPEC_TEST_EXEMPTIONS_EN 的繁体标准译名）
  '彎曲測試', '跌落測試', '防水測試', '防塵測試', '抗震測試',
  '振動測試', '溫度測試', '濕度測試', '壓力測試',
  // ③ 带宽规格上限（锚定「理論頻寬」规格名词——最高+带宽 = up to 规格陈述，非绝对化宣称）
  //   注：不补「最高#」数字锚——# 锚的 \W* 会吞中文修饰词，把「最高性能 2050MB/s」这类
  //   「营销词+规格数字」也误豁免；「最高」的规格语境护栏一律用规格名词锚（達/速度/頻寬/温度），
  //   与 v12.15 纪律一致（宁漏勿滥，未知规格名词形态待实机驱动补锚）。
  '最高理論頻寬',   // 最高理論頻寬 3200MB/s（繁体）
  '最高理论频宽',   // 简体同型
  // 红线不豁免：裸「最高」（最高性能/最高品質 无规格锚点）仍命中——
  //   「最高達#」的 '#' 数字锚与「最高讀取/寫入速度」的速度名词锚都是规格语境护栏。
]
