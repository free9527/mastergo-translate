/**
 * 全语种漏翻检测验证测试
 * 用 CSV 中的人工参考译文跑 detectUntranslatedText
 * 如果参考译文被标记为漏翻 → 100% 确认是检测 bug
 *
 * 不实际调用 LLM 翻译，只验证检测逻辑
 */
import { detectUntranslatedText, detectTargetLanguageFeatures } from '../lib/llm-api'

// ============================================================
// NM1090 PRO CSV 参考译文（13 个语种）
// ============================================================
...
const REF_TRANSLATIONS: Record<string, string[]> = {
  ...
  'ar': [
    ...
    'حتى 4 تيرابايت\n \n خيارات تخزين متنوعة: 1 تيرابايت / 2 تيرابايت / 4 تيرابايت. قدرة فائقة على التعامل مع: أنظمة التشغيل، والألعاب الكبيرة، وتخزين الوسائط بدقة UHD/8K، مما يلبي الطلب المتزايد على سعة محرك الأقراص ذي الحالة الصلبة في عصر تطبيقات الذكاء الاصطناعي والحوسبة عالية الأداء.\n \n شرائح عالية الجودة: تضمن تصميمًا سعويًا واسعًا، مما يوفر للاعبين مساحة تخزين أكبر\n السعة الفعلية القابلة للاستخدام\n محرك الأقراص ذو الحالة الصلبة 4 تيرابايت غير مكتمل السعة\n بطاقة ذاكرة NM1090 PRO سعة 4 تيرابايت\n \n *بناءً على الاختبارات الداخلية. الأداء الفعلي قد يختلف عن الأداء المذكور أو المقارن به.',
...
const ALL_LANGS = Object.keys(REF_TRANSLATIONS)

function main() {
  console.log('=== 全语种漏翻检测验证测试 ===')
  console.log(`测试语种: ${ALL_LANGS.join(', ')} (${ALL_LANGS.length} 种)`)
  console.log(`每种语种 ${EN_TEXTS.length} 条文本`)
  console.log(`用人工参考译文跑 detectUntranslatedText`)
  console.log(`参考译文被标记 = 100% 确认误判\n`)

  const glossaryMap = new Map<string, string>() // 空术语库，模拟最严格条件
  ...
  for (const lang of ALL_LANGS) {
    const refs = REF_TRANSLATIONS[lang]
    if (!refs) continue

    // 跑检测
    const untranslated = detectUntranslatedText(EN_TEXTS, refs, lang, glossaryMap)

    const fpCount = untranslated.size
    const fpRate = ((fpCount / refs.length) * 100).toFixed(0)

    console.log(`\n${'─'.repeat(50)}`)
    console.log(`${lang}: ${fpCount}/${refs.length} 条被标记漏翻 (${fpRate}% 误判率)`)

    if (fpCount > 0) {
      for (const idx of untranslated) {
        const ref = refs[idx]
        const featureCheck = detectTargetLanguageFeatures(ref, lang)
        console.log(`  [${idx}] 特征=${featureCheck.hasFeatures ? '✓' : '✗'} "${ref.slice(0, 60).replace(/\n/g, '\\n')}..."`)
      }
    } else {
      console.log(`  ✅ 无误判`)
    }
    ...
    summary.push({
      lang,
      totalTexts: refs.length,
      falsePositives: fpCount,
      fpIndices: [...untranslated],
      fpRate: `${fpRate}%`,
    })
  }
  ...
  console.log('')
  console.log('| 语言 | 文本数 | 误判数 | 误判率 | 误判索引 |')
  console.log('|------|--------|--------|--------|----------|')
  ...
  for (const s of summary) {
    ...
    totalTexts += s.totalTexts
    const status = s.falsePositives === 0 ? '✅' : '❌'
    const indices = s.fpIndices.length > 0 ? s.fpIndices.join(',') : '-'
    console.log(`| ${s.lang.padEnd(4)} | ${String(s.totalTexts).padStart(6)} | ${String(s.falsePositives).padStart(6)} | ${s.fpRate.padStart(6)} | ${indices} ${status} |`)
  }

  console.log(`\n总计: ${totalFP}/${totalTexts} 条误判 (${((totalFP / totalTexts) * 100).toFixed(1)}%)`)

  // 按误判率分组
  const severe = summary.filter(s => s.falsePositives > 0)
  const clean = summary.filter(s => s.falsePositives === 0)

  if (severe.length > 0) {
    console.log(`\n❌ 有误判的语种 (${severe.length}): ${severe.map(s => `${s.lang}(${s.fpRate})`).join(', ')}`)
  }
  if (clean.length > 0) {
    console.log(`\n✅ 无误判的语种 (${clean.length}): ${clean.map(s => s.lang).join(', ')}`)
  }
}
