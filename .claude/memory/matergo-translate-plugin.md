...
```
lib/
├── main.ts               # 插件主线程入口：扫描→翻译→应用→撤销全流程
├── llm-api.ts             # LLM API 调用：翻译批次、校对、源语言检测、内容类型分流
├── text-collector.ts      # 文本节点递归采集 + 去重合并（相同文本→合并 nodeIds）
├── default-glossary.ts    # 内置术语库（232 条，20 语种，从 Lexar术语库.csv 生成）
├── glossary-filter.ts     # 智能术语过滤：仅提取当前文本中出现的术语，减少 prompt token
├── few-shot-examples.ts   # Few-shot 翻译示例库（按语言对+内容类型索引）
├── parse-csv.ts           # CSV 解析/编码（处理逗号、引号、换行、BOM）
├── csv-handler.ts         # CSV 导入导出（翻译结果 ↔ CSV）
├── format-text.ts         # 中日韩文字间距处理
├── post-process.ts        # 译文后处理（标点、空格、格式修正）
├── constants.ts           # 全局常量（批次大小、存储 key、缓存上限等）
ui/
├── ui.ts                  # Vue 入口
...
### 2. 翻译
- 自动跳过纯数字和单字符文本
- 按 `TRANSLATE_BATCH_SIZE=10` 分批调用 LLM API
...
- 智能术语过滤：仅注入当前批次文本中出现的术语（最多 50 条）
- 支持 4 种行业翻译预设（标准版/专业版/营销版/台湾繁体版）
